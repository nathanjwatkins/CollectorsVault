 ...
 * Reads GitHub token from /home/u133725179/cv_token.txt (not in repo, never committed)
 * This solves GitHub push protection revoking tokens stored in source files.
 */

// Token stored outside webroot, outside repo — GitHub can never see it
...
// Self-update deploy scripts
foreach (['deploy.php', 'beta_deploy.php'] as $f) {
    [$c, $b] = cv_fetch($f);
    if ($c === 200 && $b) { file_put_contents($root . '/' . $f, $b); $log[] = "OK $f (" . strlen($b) . "b)"; }
